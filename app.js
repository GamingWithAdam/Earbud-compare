...
function applyTheme(theme) {
    ...
    // Re-render charts only if they exist
    if (comparisonChart) renderOrUpdateChart();
    if (scatterPlotChart) renderScatterPlot();
...
function setupEventListeners() {
    ...
    tabsContainer.addEventListener('click', (e) => {
        if (e.target.matches('.tab-link:not(.active)')) {
            ...
            // Re-render necessary components when a tab becomes visible
...
function renderDirectComparisonTable() { /* Unchanged */
...
function renderOrUpdateChart() { /* Unchanged */
    ...
    comparisonChart = new Chart(barChartCanvas, { type: 'bar', data: { labels: dataToDisplay.map(p => p.name), datasets: [{ label: chartCategorySelector.options[chartCategorySelector.selectedIndex].text, data: dataToDisplay.map(p => p.scores[category]), backgroundColor: 'rgba(0, 123, 255, 0.6)', borderColor: 'rgba(0, 123, 255, 1)', borderWidth: 1 }] },
        options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, onClick: (e) => { const pts = comparisonChart.getElementsAtEventForMode(e, 'nearest', { intersect: true }, true); if (pts.length) openQuickViewModal(dataToDisplay[pts[0].index].id); }, onHover: (e, el) => e.native.target.style.cursor = el[0] ? 'pointer' : 'default', plugins: { legend: { display: false } }, scales: { x: { ticks: { color: 'var(--secondary-text-color)' }, grid: { color: 'var(--card-border-color)' } }, y: { ticks: { color: 'var(--secondary-text-color)', autoSkip: false }, grid: { display: false } } } }
    });
}

function renderScatterPlot() { /* Unchanged */
    let dataToDisplay = getFilteredData();
    ...
    scatterPlotChart = new Chart(scatterPlotCanvas, { type: 'scatter', data: { datasets: [{ label: 'Price vs. Sound', data: plotData, backgroundColor: 'rgba(0, 123, 255, 0.7)' }] },
        options: { responsive: true, maintainAspectRatio: false, onClick: (e) => { const pts = scatterPlotChart.getElementsAtEventForMode(e, 'nearest', { intersect: true }, true); if (pts.length) openQuickViewModal(plotData[pts[0].index].id); }, onHover: (e, el) => e.native.target.style.cursor = el[0] ? 'pointer' : 'default', plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c) => `${c.raw.label} ($${c.raw.x}, Score: ${c.raw.y})` } } },
            scales: { x: { title: { display: true, text: 'Price (USD)', color: 'var(--primary-text-color)' }, ticks: { color: 'var(--secondary-text-color)' }, grid: { color: 'var(--card-border-color)' } }, y: { title: { display: true, text: 'Sound Score', color: 'var(--primary-text-color)' }, ticks: { color: 'var(--secondary-text-color)' }, grid: { color: 'var(--card-border-color)' } } }
        }
    });
}

function renderDataTable(searchTerm = '') { /* Unchanged */
    let dataToDisplay = getFilteredData();
